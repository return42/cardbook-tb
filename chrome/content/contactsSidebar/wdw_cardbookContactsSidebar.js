	...
	var wdw_cardbookContactsSidebar = {
		...
		search: function () {
			...
			for (var i = 0; i < cardbookRepository.cardbookAccounts.length; i++) {
				if (cardbookRepository.cardbookAccounts[i][1] && cardbookRepository.cardbookAccounts[i][5]) {
					...
					if (cardbookRepository.cardbookAccounts[i][6] != "SEARCH") {
						if ((searchAB == myDirPrefId) || (searchAB === "allAddressBooks")) {
			...
			if (!prefs.getBoolPref("extensions.cardbook.exclusive")) {
				...
				while ( contacts.hasMoreElements() ) {
					...
					if ((searchAB == contact.dirPrefId) || (searchAB === "allAddressBooks")) {
							...
							if (!myABCard.isMailList) {
								if (myPrimaryEmail != "") {
									...
									if (lSearchString.indexOf(searchInput) >= 0 || searchInput == "") {
										...
										wdw_cardbookContactsSidebar.searchResults.push([myDisplayName, contact.dirName, myPrimaryEmail, false, "CARDCORE", myABCard, MailServices.headerParser.makeMimeAddress(myDisplayName, myPrimaryEmail)]);
									}
								}
							} else {
								...
								lSearchString = lSearchString.replace(/[\s+\-+\.+\,+\;+]/g, "").toUpperCase();
								if (lSearchString.indexOf(searchInput) >= 0 || searchInput == "") {
									wdw_cardbookContactsSidebar.searchResults.push([myDisplayName, contact.dirName, "", true, "LISTCORE", myABCard, MailServices.headerParser.makeMimeAddress(myDisplayName, myDisplayName)]);
								}
							}
						}
					}
		...
		getSelectedCards: function () {
			...
			for (var i = 0; i < numRanges; i++) {
				myTree.view.selection.getRangeAt(i,start,end);
				for (var j = start.value; j <= end.value; j++){
					listOfUid.push([wdw_cardbookContactsSidebar.searchResults[j][4], wdw_cardbookContactsSidebar.searchResults[j][5]]);
				}
			}
			return listOfUid;
		},
		...
		deleteCard: function () {
			var AB =  MailServices.ab.getDirectoryFromId(document.getElementById('CardBookABMenulist').value);
		...
		onABChange: function () {
			...
			cardbookElementTools.loadCategories("categoriesMenupopup", "categoriesMenulist", ABDefaultValue, categoryDefaultValue, true, true, true);
